...
class BankEditingScreenState extends State<BankEditingScreen> {
  ...
  TextEditingController? ifscControlletr ;
  ...
  final FocusNode _ifsc = FocusNode();
  ...
  _updateUserAccount() async {
    ...
    String ifsc = ifscControlletr!.text.trim();
    ...
    if(Provider.of<BankInfoProvider>(context, listen: false).bankInfo!.bankName == _bankNameController!.text
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("IFSC Code is "),
          backgroundColor: ColorResources.red));
    }else {
      ...
      updateUserInfoModel.ifsc = ifscControlletr?.text ?? '';
      ...
      SellerBody sellerBody = SellerBody(
        ...
        holderName: _holderNameController?.text ?? '',ifsc: ifscControlletr?.text ?? ''
      );
